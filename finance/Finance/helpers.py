...
def get_stock_data(symbol):
    ...
    try:
        ...
        url = f"https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol={urllib.parse.quote_plus(symbol)}&interval=1min&apikey={api_key}"
        response = requests.get(url)
        response.raise_for_status()
        return response.json()  # Restituisci i dati JSON ricevuti
